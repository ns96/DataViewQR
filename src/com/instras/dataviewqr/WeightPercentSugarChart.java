 ...
 * @author nathan
 */
public class WeightPercentSugarChart extends DataViewChart {
    
    private final String title = "Weight Percent Sugar Data";
    
    public WeightPercentSugarChart() {
    ...
    public Form getChart(XYData xyData) {
        ...
        List<double[]> standardList = getStandardDataList();
        
        Component[] tabs = new Component[2];
        String[] tabTitles = {"Wt%/Density", "Results"};
        
        tabs[0] = getConcentrationVsPhChart(standardList);
    ...
    private Component getConcentrationVsPhChart(List<double[]> dataList) {
        ...
        setChartSettings(renderer, "", "wt%", "g/mL", (limits[0] - 0.1), (limits[1] + 0.1), (limits[2] - 0.1), (limits[3] + 0.1),
    ...
    /**
     * Generate the list that 
     * @return 
     */
    private List<double[]> getStandardDataList() {
        ArrayList<double[]> dataList = new ArrayList<double[]>();
        ...
        for(int i = 0; i < xdata.length; i++) {
            xdata[i] =(double)i;
            ydata[i] = 100/((xdata[i]/1.5805) + ((100-xdata[i])/1.002));
        }
